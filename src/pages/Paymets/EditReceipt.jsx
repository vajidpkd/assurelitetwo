...
const EditReceipt = () => {
    ...
    const fetchPartyOptions = async () => {
      try {
        ...
        if (initialValues.Party) {
          ...
          setSelectedParty(response[0].AccountName); 
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchPaymentDetails = async () => {
      try {
        const response = await axios.get(`https://assurelite.datacubeglobal.com/Api/index.php/Entries/get_receiptByRefNo?RefNo=${RefNo}`);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const response = await axios.post('https://assurelite.datacubeglobal.com/Api/index.php/Entries/update_Receipt', data, {
        headers: {
