...
import { getAllSalesQuotation } from 'api/apis/accountmaster';

export default function MasterSalesQuotation() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const data = await getAllSalesQuotation();
        console.log(data);
      ...
      cell: (row) => (
        <div className="flex">
          ...
          <div
            ...
            onClick={() => handleDeleteQuotation(row.RefNo)}
          >
            <DeleteOutlined />
          </div>
        </div>
  ...
  const handleDeleteQuotation = async (RefNo) => {
    try {
      const response = await axios.get(`https://assurelite.datacubeglobal.com/Api/index.php/Entries/delete_Quotation?RefNo=${RefNo}`);
      console.log('Delete Unit Response:', response);
      // const updatedUnits = await getAllSalesQuotation();
      ...
      <MainCard>
        <div className="container mx-auto rounded-md bg-white ">
          <div className="flex justify-between">
            <Typography className="font-semibold text-2xl">Sales Quotation</Typography>
            <Button variant="contained" onClick={() => navigate('/Addsalesquotation')} size="small">
              + New
            </Button>
          </div>
          <div className="py-3">
