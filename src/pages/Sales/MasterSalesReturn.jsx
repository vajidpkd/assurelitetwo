...
import { getAllSalesReturn } from 'api/apis/accountmaster';

export default function MasterSalesReturn() {
      ...
      cell: (row) => (
        <div className="flex">
          ...
          <div
            ...
            onClick={() => handleDeleteReturn(row.RefNo)}
          >
            <DeleteOutlined />
          </div>
        </div>
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const data = await getAllSalesReturn();
        console.log(data);
  ...
  const handleDeleteReturn = async (RefNo) => {
    try {
      const response = await axios.get(`https://assurelite.datacubeglobal.com/Api/index.php/Entries/delete_SalesReturn?RefNo=${RefNo}`);
      console.log('Delete Unit Response:', response);
      const updatedUnits = await getAllSalesReturn();
      ...
      <MainCard>
        <div className="container mx-auto rounded-md bg-white ">
          <div className="flex justify-between">
            <Typography className="font-semibold text-2xl">Sales Return</Typography>
            <Button variant="contained" onClick={() => navigate('/Addsalesreturn')} size="small">
              + New
            </Button>
          </div>
