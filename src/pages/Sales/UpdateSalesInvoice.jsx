...
//   console.log(invoiceData.Received, 'receioved');
...
//   useEffect(() => {
//     calculateTotals();
//   }, [Items, invoiceData.OtherCharge, selectedTaxType, invoiceData.Discount, invoiceData.MultiReceipt, invoiceData.GrandTotal]);
...
//       // Convert payload without PaymentType to JSON string
...
//       const response = await axios.post('https://assurelite.datacubeglobal.com/Api/index.php/Entries/update_bill', data, {
//         headers: {
...
//     setSelectedProduct(selectedOption); // Ensure to use the correct variable
//   };
...
//         console.log('****************', data);
//         const uniqueMultiReceipts = Array.from(new Set(data.MultiReceipt.map(JSON.stringify))).map(JSON.parse);
//         setInvoiceData({
...
//           MultiReceipt: uniqueMultiReceipts || [],
//           Items: data.Items || [],
//           RefNo: data.RefNo || '',
//           UserID: 1,
//           CompanyID: 1
...
//         //   const item = data.Items[0]; // Assuming only one item for simplicity
//         //   const selected = product.find((p) => p.ItemID === item.ItemID);
//         //   if (selected) {
//         //     setSelectedProduct({ label: selected.ItemName, id: selected.ItemID });
//         //   }
//         // }
//         const bType = data.BType;
//         const mappedType = bType === 'LOCAL' ? 'regular' : bType === 'INTERSTATE' ? 'inter' : 'unregistered';
//         setSelectedTaxType(mappedType);
//       } catch (error) {
...
//   //         const selected = product.find((p) => p.id === data.SelectedProductID);
...
//   //   setInvoiceData({ ...invoiceData, Received: e.target.value });
//   // };

//   // useEffect(() => {
//   //   // Assuming you're fetching invoiceData from an API or similar
//   //   // Make sure to handle the case where Total might be undefined or null
//   //   // and convert it to a number
//   //   setInvoiceData(prevData => ({
//   //     ...prevData,
//   //     Total: prevData.Total ?? 0 // Ensure Total is a number
//   //   }));
//   // }, []);
...
//   //   setInvoiceData(prevData => ({
//   //     ...prevData,
//   //     GrandTotal: parseFloat(grandTotal) // Ensure GrandTotal is a number
//   //   }));
//   // }, [invoiceData.Items, selectedTaxType, invoiceData.OtherCharge, invoiceData.Discount]);
...
//                                   }}
//                                   getOptionLabel={(option) => option.label}
//                                   value={taxtypes.find((type) => type.value === selectedTaxType) || null}
//                                   onChange={(event, newValue) => {
//                                     setSelectedTaxType(newValue ? newValue.value : '');
//                                   }}
...
//                                 </td>
//                                 <td id="MainTotal">
//                                   {/* <h5>{typeof invoiceData.GrandTotal === 'number' ? invoiceData.GrandTotal.toFixed(2) : '0.00'}</h5> */}
//                                   <h5>{invoiceData.Total ? invoiceData.Total : ''}</h5>

//                                   {/* <h5>{invoiceData.Gross ? invoiceData.Gross : ''}</h5> */}
//                                 </td>
//                               </tr>
//                             </tbody>
//                           </table>
...
function UpdateSalesInvoice() {
  ...
  const calculateTotals = () => {
    ...
    let totalDiscPercent = 0;
    // let totalMultiReceipt = 0;
    let receivedAmount = parseFloat(invoiceData.Received) || 0;
    // let totalTax = 0;
    let totalOtherCharges = parseFloat(invoiceData.OtherCharge) || '';
    ...
    // });
    // invoiceData.MultiReceipt.forEach((receipt) => {
    //   receivedAmount += parseFloat(receipt.Amount) || 0; // Add amounts correctly
    // });
  ...
  useEffect(() => {
    calculateTotals();
  }, [Items, invoiceData.OtherCharge, selectedTaxType, invoiceData.Discount, invoiceData.Receipt, invoiceData.GrandTotal]);
  ...
  const handleProductSelect = (event, newValue, index) => {
    const updatedItems = [...Items];
    const selectedItem = product.find((p) => p.ItemName === newValue.label);
    ...
    if (selectedItem) {
      updatedItems[index] = {
        ...
        SRate: selectedItem.SRate || 0,
        Unit: selectedItem.UnitName || '',
        CGST: selectedItem.CGST || 0,
        IGST: selectedItem.IGST || 0,
        SGST: selectedItem.SGST || 0,
        Tax: selectedItem.Tax || 0,
        Cess: selectedItem.Cess || 0,
        Qty: updatedItems[index].Qty || 1,
        Gross: (updatedItems[index].Qty * selectedItem.SRate).toFixed(2),
        DiscPer: updatedItems[index].DiscPer || 0,
        Disc: ((updatedItems[index].Gross * (updatedItems[index].DiscPer || 0)) / 100).toFixed(2),
        Net: (updatedItems[index].Gross - updatedItems[index].Disc).toFixed(2),
        Total: (parseFloat(updatedItems[index].Net) + (parseFloat(updatedItems[index].Net) * (selectedItem.Tax || 0)) / 100).toFixed(2)
      };
    } else {
      updatedItems[index] = {
        ...updatedItems[index],
    ...
    setSelectedProduct((prev) => {
      ...
      newSelectedProduct[index] = newValue;
  ...
  useEffect(() => {
    const fetchPaymentDetails = async () => {
      try {
        ...
        console.log(data.Received, '>>>>>>>>>>>>>>>>>>>>>>>>');
        setItems(data.Items || []);
        const initialSelectedProducts = data.Items.map((item) => {
          const selectedProduct = product.find((p) => p.ItemID === item.ItemID);
          return selectedProduct ? { label: selectedProduct.ItemName, id: selectedProduct.ItemID } : null;
        });
        // setSelectedProducts(initialSelectedProducts);
        setSelectedProduct(initialSelectedProducts);
        // if (data.Items && data.Items.length > 0) {
        //   const item = data.Items[0]; // Assuming only one item for simplicity
        //   const selected = product.find((p) => p.ItemID === item.ItemID);
        //   if (selected) {
